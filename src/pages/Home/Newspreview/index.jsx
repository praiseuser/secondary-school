...
const FeaturedCard = ({ item, visible }) => {
    ...
    return (
        <Box
            onClick={() => navigate(`/news/${item.slug}`)}   // ← click goes to article
            ...
            sx={{
                ...
                boxShadow: hovered ? '0 32px 80px rgba(0,0,0,0.35)' : '0 8px 32px rgba(0,0,0,0.18)',
        ...
        >
            ...
            <Box sx={{
                position: 'absolute', inset: 0,
                background: `linear-gradient(180deg, rgba(10,20,50,0.15) 0%, rgba(10,20,50,0.5) 40%, rgba(10,20,50,0.95) 100%)`,
            }} />
            ...
            <Box sx={{ position: 'absolute', bottom: 0, left: 0, right: 0, p: { xs: 3, md: 4 } }}>
                <Stack direction="row" gap={2} alignItems="center" sx={{ mb: 1.5 }}>
                    <Stack direction="row" alignItems="center" gap={0.6}>
                        ...
                        <Typography sx={{ fontFamily: typography.fontFamily.body, fontSize: typography.fontSize.xs, color: 'rgba(255,255,255,0.5)' }}>{item.date}</Typography>
                    </Stack>
                    <Box sx={{ width: 3, height: 3, borderRadius: '50%', bgcolor: 'rgba(255,255,255,0.3)' }} />
                    <Typography sx={{ fontFamily: typography.fontFamily.body, fontSize: typography.fontSize.xs, color: 'rgba(255,255,255,0.5)' }}>{item.readTime}</Typography>
                </Stack>

                <Typography sx={{ fontFamily: typography.fontFamily.accent, fontSize: { xs: typography.fontSize.xl, md: typography.fontSize['2xl'] }, fontWeight: typography.fontWeight.bold, color: colors.text.light, lineHeight: 1.25, mb: 1.5 }}>
                    {item.title}
                </Typography>
                <Typography sx={{ fontFamily: typography.fontFamily.body, fontSize: typography.fontSize.sm, color: 'rgba(255,255,255,0.65)', lineHeight: 1.7, mb: 2.5, display: { xs: 'none', md: 'block' } }}>
                    {item.excerpt}
                </Typography>

                <Stack direction="row" alignItems="center" gap={1}
                    sx={{ transform: hovered ? 'translateX(6px)' : 'translateX(0)', transition: 'transform 0.3s ease' }}>
...
const SmallCard = ({ item, index, visible }) => {
    ...
    return (
        ...
        >
            {/* Left color bar */}
            <Box sx={{ width: hovered ? 6 : 4, flexShrink: 0, bgcolor: tag.bg, borderRadius: '14px 0 0 14px', transition: 'width 0.3s ease' }} />
            ...
            <Box sx={{ py: 2, pr: 2, display: 'flex', flexDirection: 'column', justifyContent: 'center', flexGrow: 1 }}>
                <Stack direction="row" alignItems="center" gap={0.6} sx={{ mb: 1 }}>
                    ...
                    <Typography sx={{ fontFamily: typography.fontFamily.body, fontSize: '0.68rem', fontWeight: typography.fontWeight.bold, color: tag.bg, letterSpacing: 1, textTransform: 'uppercase' }}>
                        {item.tag}
                    </Typography>
                </Stack>
                <Typography sx={{ fontFamily: typography.fontFamily.heading, fontSize: typography.fontSize.sm, fontWeight: typography.fontWeight.semiBold, color: hovered ? colors.primary.main : colors.text.primary, lineHeight: 1.4, mb: 1, transition: 'color 0.3s ease' }}>
                    {item.title}
                </Typography>
                <Stack direction="row" alignItems="center" gap={0.6}>
                    <CalendarTodayIcon sx={{ fontSize: 10, color: colors.text.disabled }} />
                    <Typography sx={{ fontFamily: typography.fontFamily.body, fontSize: '0.7rem', color: colors.text.disabled }}>{item.date}</Typography>
                </Stack>
            </Box>
        </Box>
    );
...
const NewsPreview = () => {
    ...
    return (
        <>
            ...
            <Box ref={ref} sx={{ position: 'relative', bgcolor: colors.primary.dark, py: { xs: 8, md: 12 }, overflow: 'hidden' }}>

                {/* Faded BG watermark */}
                <Typography sx={{ position: 'absolute', top: '50%', left: '-2%', transform: 'translateY(-50%)', fontFamily: typography.fontFamily.accent, fontSize: { xs: '8rem', md: '14rem' }, fontWeight: typography.fontWeight.black, color: colors.primary.light, opacity: 0.04, lineHeight: 1, userSelect: 'none', pointerEvents: 'none', zIndex: 0 }}>
                    NEWS
                </Typography>
                ...
                <Box sx={{ position: 'relative', zIndex: 1, maxWidth: 1400, mx: 'auto', px: { xs: 3, md: 6 } }}>
                    ...
                    <Box sx={{
                        ...
                        <Box>
                            ...
                            <Typography sx={{ fontFamily: typography.fontFamily.accent, fontSize: { xs: typography.fontSize['2xl'], md: '2.6rem' }, fontWeight: typography.fontWeight.bold, color: colors.text.light, lineHeight: 1.2 }}>
                                Stories from the
                    ...
                    <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', md: '1fr 1fr' }, gap: 3, alignItems: 'stretch' }}>
                        ...
                        <Stack gap={3} justifyContent="space-between">
                            ...
                            >
                                <Box>
                                    <Typography className="cta-text" sx={{ fontFamily: typography.fontFamily.heading, fontSize: typography.fontSize.md, fontWeight: typography.fontWeight.bold, color: colors.text.light, mb: 0.4, transition: 'color 0.3s ease' }}>
                                        Never miss a story
                                    </Typography>
                                    <Typography className="cta-sub" sx={{ fontFamily: typography.fontFamily.body, fontSize: typography.fontSize.xs, color: 'rgba(255,255,255,0.55)', transition: 'color 0.3s ease' }}>
                                        Click to see all our latest news & events
                                    </Typography>
                                </Box>
                                <Box className="cta-arrow" sx={{ width: 40, height: 40, borderRadius: '50%', bgcolor: `${colors.secondary.main}22`, border: `1px solid ${colors.secondary.main}55`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, transition: 'all 0.3s ease' }}>
